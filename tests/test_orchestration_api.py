...
# def test_vault_service_kv_secret() -> None:
#     """
#     Verify that the Vault dev instance is running and that KV secret is properly written.
#     This is critical for the LLM Orchestration Service to resolve configuration.
#     """
...
#     kv_path = "secret/data/llm/connections/azure_openai/production/gpt-4o-mini"
#     url = f"{vault_addr}/v1/{kv_path}"

#     response = requests.get(url, headers=headers, timeout=10)
#     assert response.status_code == 200, f"Vault KV read failed: {response.status_code} - {response.text}"

#     data = response.json()["data"]["data"]
#     expected_fields = ["connection_id", "endpoint", "api_key", "deployment_name", "model"]

#     for field in expected_fields:
#         assert field in data, f"Missing field in Vault secret: {field}"

#     # Assert values from env (set by GitHub Actions or defaulted)
#     assert data["api_key"] == os.getenv("AZURE_OPENAI_API_KEY", "TEST_API_KEY")
#     assert data["deployment_name"] == os.getenv("AZURE_OPENAI_DEPLOYMENT", "gpt-4o-mini")

#     logger.info("✅ Vault KV secret test passed.")
...
def test_vault_service_kv_secret(rag_stack: Any) -> None:
    """
    Verify that the Vault dev instance is running and that KV secret is properly written.
    This is critical for the LLM Orchestration Service to resolve configuration.
    
    IMPORTANT: This test now uses rag_stack.get_vault_url() to get the mapped localhost URL
    instead of using 'http://vault:8200' which only works inside the Docker network.
    """
    # Get the mapped Vault URL (e.g., http://127.0.0.1:32768)
    # This resolves the DNS issue where 'vault' hostname is not accessible from test host
    ...
    logger.info(f"📡 Testing Vault KV read at: {url}")
    response = requests.get(url, headers=headers, timeout=10)
    assert response.status_code == 200, f"Vault KV read failed: {response.status_code} - {response.text}"
...
def test_rag_stack_services_available(rag_stack: Any) -> None:
    ...
    assert rag_stack.is_service_available("qdrant"), "Qdrant is not available"
    assert rag_stack.is_service_available("llm-orchestration-service"), "Orchestration service is not available"
    assert rag_stack.is_service_available("vault"), "Vault is not available"
    ...
    assert qdrant_url.startswith("http://"), f"Unexpected Qdrant URL: {qdrant_url}"
    assert orchestration_url.startswith("http://"), f"Unexpected Orchestration URL: {orchestration_url}"
    assert vault_url.startswith("http://"), f"Unexpected Vault URL: {vault_url}"
