...
def generate_detailed_results_table(results: Dict[str, Any]) -> str:
    ...
    for result in results["detailed_results"]:
        ...
        test_status = "PASS" if passed_metrics >= 4 else "FAIL"  # 4 out of 5 metrics need to pass
...
def generate_failure_analysis(results: Dict[str, Any]) -> str:
    ...
    for failure in failed_results[:10]:  # Limit to first 10 failures
        query_preview = failure["input"][:50] + "..." if len(failure["input"]) > 50 else failure["input"]
        reason_preview = failure["reason"][:100] + "..." if len(failure["reason"]) > 100 else failure["reason"]
...
def generate_recommendations(results: Dict[str, Any]) -> str:
    ...
    low_performing_metrics = [
        (metric, score) for metric, score in avg_scores.items() 
        if score < 0.7
    ]
    
    if not low_performing_metrics:
        recommendations += "All metrics are performing well above the threshold of 0.7. Great job!\n\n"
...
def save_report_to_file(results: Dict[str, Any], output_path: str = "test_report.md") -> str:
