...
class TestRAGSystem:
    ...
    def setup_class(cls):
        ...
        cls.contextual_precision: ContextualPrecisionMetric = ContextualPrecisionMetric(threshold=0.7)
        cls.contextual_recall: ContextualRecallMetric = ContextualRecallMetric(threshold=0.7)
        cls.contextual_relevancy: ContextualRelevancyMetric = ContextualRelevancyMetric(threshold=0.7)
        cls.answer_relevancy: AnswerRelevancyMetric = AnswerRelevancyMetric(threshold=0.7)
    ...
    def _create_test_case(self, data_item: Dict[str, Any], provider: str = "anthropic") -> LLMTestCase:
    ...
    @pytest.mark.parametrize("test_item", [
        item for item in json.load(
            open(Path(__file__).parent.parent / "data" / "test_dataset.json", 'r', encoding='utf-8')
        )
    ])
