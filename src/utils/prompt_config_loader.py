        ...
        with self._cache_lock:
            ...
            try:
                ...
                if prompt_text:
                    ...
                    logger.warning("⚠️ No prompt configuration found in database")
                ...
                if self._cached_prompt:
                    logger.warning(
                        f"⚠️ Using stale cache (age: {self._get_cache_age():.1f}s)"
                    )
                return self._cached_prompt or ""
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if isinstance(data, list) and len(data) > 0:
                        ...
                        logger.warning(f"⚠️ Unexpected data type: {type(data)}, value: {data}")
                    
                    logger.info(f"📦 Extracted prompt length: {len(prompt) if prompt else 0}")
                    
                    if prompt:
                        logger.info(
                            f"✅ Loaded prompt on attempt {attempt} "
                            f"({len(prompt)} chars)"
                        )
                        return prompt
                    else:
                        logger.warning(
                            f"⚠️ Prompt field is empty (attempt {attempt})"
                        )
                        return None  # Database has no configuration
                
                else:
                    logger.warning(
                        f"⚠️ HTTP {response.status_code} on attempt {attempt}: "
                        f"{response.text[:200]}"
                    )
                    
            except requests.exceptions.Timeout:
                logger.warning(
                    f"⏱️ Request timeout on attempt {attempt} "
                ...
                logger.error(
                    f"❌ Invalid response format on attempt {attempt}: {e}"
                )
