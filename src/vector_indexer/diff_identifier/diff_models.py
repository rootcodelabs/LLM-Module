...
class DiffResult(BaseModel):
    ...
    total_files_scanned: int = Field(..., description="Total files found in current scan")
    previously_processed_count: int = Field(..., description="Number of previously processed files")
    is_first_run: bool = Field(..., description="Whether this is the first time running")
...
class VersionState(BaseModel):
    ...
    processed_files: Dict[str, ProcessedFileInfo] = Field(..., description="Map of hash to file info")
...
class DiffConfig(BaseModel):
    ...
    metadata_filename: str = Field(default="processed-metadata.json", description="Metadata file name")
    ...
    max_delay_seconds: int = Field(default=8, description="Maximum delay between retries")
